...
```{r data import}
# import data sets
median_sale_price <- readr::read_csv("data/median_sale_price.csv")
...
ny_housing_data <- ny_mean_sfr_value %>% 
  ...
  dplyr::left_join(rental_costs, by = "Date") %>% 
  dplyr::left_join(total_crime_commissions_monthly, by = "Date")

ny_housing_data <- ny_housing_data %>% 
  dplyr::select(-RegionName.x, - RegionName.y)
